...
function parseElement(context, ancestors) {
  ...
  if (startsWithEndTagOpen(context.source, element.tag)) {
    ...
    throw new Error(`确实结束标签: ${element.tag}`)
  }
...
function parseTag(context: any, type: TagType): any {
  ...
  // 暂时不处理 selfClose 标签的情况，所以可以直接 advanceBy 1 个光标 < 的 下一个就是 >
  advanceBy(context, 1)
...
function parseText(context): any {
  ...
  for (let i = 0; i < endTokens.length; i++) {
    ...
    // endIndex > index 是需要 endIndex 尽可能的小
    // 比如说
    // hi, {{ 123 }} <div></div>
    // 那么这里就应该停到 {{ 这里，而不是停到 }}
  }
...
function parseTextData(context, length: number): any {
